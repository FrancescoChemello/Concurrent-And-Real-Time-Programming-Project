...
#define MAX_FRAME_SIZE 1280*720*3
...
struct msgframe{
    ...
    char type[5];               // the type (MJPG or YUYV)
...
unsigned char * mmap_frame;             // frame in memory map
int vd, timer;
int msgId;
...
static void frame_producer(int frame_size, char frtype[5]){   
    ...
    printf("Size of the frame in input: %d\n", frame_size);
    ...
    while(difftime(time(0), start) < timer){
        ...
        mmap_frame = mmap(NULL, frame_size, PROT_READ | PROT_WRITE, MAP_SHARED, memsh, 0);
